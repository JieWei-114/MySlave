...
def extract_entities_nlp(text: str) -> list[str]:
    ...
    try:
        ...
        relevant_labels = {'PERSON', 'ORG', 'GPE', 'PRODUCT', 'EVENT', 'WORK_OF_ART', 'LAW', 'NORP', 'FAC'}
        ...
        logger.debug(
            'NLP entity extraction: %s entities from %s chars',
            len(entities),
            len(text)
        )
...
def validate_entities(
    ...
    factual_blocks: list[str] | None = None
) -> list[str]:
    """
    ...
    if factual_blocks:
        context_text = '\n\n'.join(factual_blocks)
        logger.debug(
            'Using FACTUAL blocks for validation (len=%s)',
            len(context_text)
        )
    elif context_blocks:
        context_text = '\n\n'.join(context_blocks)
        logger.debug(
            'Using all context blocks for validation (len=%s)',
            len(context_text)
        )
    else:
...
def detect_uncertainty(
    ...
    uncertainty_patterns = [
        ...
        r'\bunclear\b', r'\bassume\b', r'I\'m not sure',
        r'uncertain', r'confusing'
    ]
